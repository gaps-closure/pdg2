...
use llvm_ir::{
    Constant, Function, HasDebugLoc, Instruction, Module, Name, Operand, Terminator, Type, TypeRef,
};
    ...
    module
        ...
        .flat_map(|f| {
            f.basic_blocks
                ...
                .flat_map(|b| &b.instrs)
                .filter_map(move |i| {
                    if let Instruction::Call(c) = i {
                        Some((f.clone(), c.clone()))
                    } else {
                        None
                    }
                })
...
pub fn call_to_function_pointer(call: &Call) -> bool {
    call.function.as_ref().right().map_or(false, |o| {
...
pub fn args_has_debug_info(f: &Function, c: &Call) -> bool {
    ...
    c.arguments.iter().all(|(op, _)| match op {
        Operand::LocalOperand { name, ty: _ } => debug_set.get(name).is_some(),
        _ => true,
    })
}
...
impl Display for LLID {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::GlobalName { global_name } => f.write_fmt(format_args!("@{}", global_name)),
            Self::LocalName {
                global_name,
                local_name,
...
pub enum LLItem {
    ...
    Parameter {
        function: llvm_ir::function::Function,
        index: usize,
        parameter: llvm_ir::function::Parameter,
    },
...
impl LLItem {
    ...
    pub fn constant_call_name(&self) -> Option<Name> {
        self.call()
            .and_then(|c| c.function.right())
            .and_then(|o| match o {
                Operand::ConstantOperand(g) => match g.as_ref() {
                    Constant::GlobalReference { name, ty: _ } => Some(name.to_owned()),
                    _ => None,
                },
                _ => None,
            })
...
impl Eq for LLValue {}
impl PartialOrd for LLValue {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.id.partial_cmp(&other.id)
    }
}
impl Ord for LLValue {
...
pub fn callee_name(call: &Call) -> Option<Name> {
    call.function
        ...
        .and_then(|op| match op {
            ...
            Operand::ConstantOperand(_) => match op.as_constant().unwrap() {
...
impl<T: FunctionUsedAsPointer> FunctionUsedAsPointer for Vec<T> {
    fn funcs_used_as_pointer(&self) -> HashSet<Name> {
        self.iter()
            .flat_map(|e| e.funcs_used_as_pointer())
            .collect()
    }
}
...
impl FunctionUsedAsPointer for llvm_ir::BasicBlock {
    fn funcs_used_as_pointer(&self) -> HashSet<Name> {
        union!(
            self.instrs.funcs_used_as_pointer(),
            self.term.funcs_used_as_pointer()
        )
    }
}
    ...
    fn funcs_used_as_pointer(&self) -> HashSet<Name> {
        ...
        match self {
            Terminator::Ret(r) => r
                .return_operand
                .as_ref()
                .map(|r| r.funcs_used_as_pointer())
                .unwrap_or(empty),
            Terminator::Br(_) => empty,
            Terminator::CondBr(b) => b.condition.funcs_used_as_pointer(),
            Terminator::Switch(s) => union!(
                s.operand.funcs_used_as_pointer(),
                s.dests.iter().flat_map(|(c, _)| c.funcs_used_as_pointer())
            ),
            Terminator::IndirectBr(b) => b.operand.funcs_used_as_pointer(),
            Terminator::Invoke(i) => i
                .arguments
                .iter()
                .flat_map(|(o, _)| o.funcs_used_as_pointer())
                .collect(),
...
impl FunctionUsedAsPointer for llvm_ir::Instruction {
    fn funcs_used_as_pointer(&self) -> HashSet<Name> {
        ...
        match self {
            ...
            Instruction::Load(x) => x.address.funcs_used_as_pointer(),
            Instruction::Store(x) => union!(
                x.address.funcs_used_as_pointer(),
                x.value.funcs_used_as_pointer()
            ),
            ...
            Instruction::GetElementPtr(x) => x.address.funcs_used_as_pointer(),
            // Instruction::Trunc(x) =>
            // x.operand.funcs_used_as_pointer(),
            // Instruction::ZExt(x) =>
            // x.operand.funcs_used_as_pointer(),
            ...
            Instruction::Call(c) => c
                .arguments
                .iter()
                .flat_map(|(c, _)| c.funcs_used_as_pointer())
                .collect(),
            // Instruction::VAArg(x) =>
    ...
    fn funcs_used_as_pointer(&self) -> HashSet<Name> {
        ...
        match self {
            ...
            llvm_ir::Constant::Struct {
                ...
                .iter()
                .flat_map(|c| c.funcs_used_as_pointer())
                .collect::<HashSet<_>>(),
            llvm_ir::Constant::Array {
                ...
                .flat_map(|c| c.funcs_used_as_pointer())
                .collect::<HashSet<_>>(),
            llvm_ir::Constant::Vector(elements) => elements
                .iter()
                .flat_map(|c| c.funcs_used_as_pointer())
                .collect::<HashSet<_>>(),
            llvm_ir::Constant::BitCast(x) => x.operand.funcs_used_as_pointer(),
            _ => empty,
        }
        // match self {
...
impl Names for GlobalVariable {
    fn names(&self) -> HashSet<Name> {
        self.initializer
            .iter()
            .flat_map(|x| x.as_ref().names())
            .collect()
    }
}
    ...
    fn names(&self) -> HashSet<Name> {
        ...
        match self {
            ...
            Constant::Struct {
                ...
                values,
                is_packed: _,
            } => values.iter().flat_map(|x| x.names()).collect(),
            Constant::Array {
                element_type: _,
                elements,
            } => elements.iter().flat_map(|x| x.names()).collect(),
            ...
            Constant::InsertElement(x) => {
                union!(x.vector.names(), x.element.names(), x.index.names())
            }
            Constant::ShuffleVector(x) => {
                union!(x.mask.names(), x.operand0.names(), x.operand1.names())
            }
            Constant::ExtractValue(x) => x.aggregate.names(),
            Constant::InsertValue(x) => union!(x.element.names(), x.aggregate.names()),
            Constant::GetElementPtr(x) => {
                union!(x.address.names(), x.indices.iter().flat_map(|x| x.names()))
            }
...
impl Names for Instruction {
    fn names(&self) -> HashSet<Name> {
        ...
        match self {
            ...
            Instruction::Load(x) => union!(
                x.address.names(),
                HashSet::<Name>::from_iter([x.dest.to_owned()])
            ),
            Instruction::Store(x) => union!(x.value.names(), x.address.names()),
            Instruction::Fence(_) => empty,
            Instruction::CmpXchg(x) => union!(
                x.address.names(),
                x.expected.names(),
                x.replacement.names(),
                x.dest.names()
            ...
            Instruction::GetElementPtr(x) => union!(
                x.address.names(),
                x.indices.iter().flat_map(|x| x.names()),
                x.dest.names()
            ),
            ...
            Instruction::Phi(x) => union!(
                x.dest.names(),
                x.incoming_values
                    .iter()
                    .flat_map(|(op, n)| union!(op.names(), n.names()))
            ),
            Instruction::Select(x) => union!(
                x.condition.names(),
                x.false_value.names(),
                x.true_value.names()
            ),
            Instruction::Freeze(x) => union!(x.dest.names(), x.operand.names()),
            Instruction::Call(x) => union!(
                x.dest.as_ref().map(|x| x.names()).unwrap_or(empty),
            ...
            Instruction::CatchPad(x) => union!(
                x.dest.names(),
                x.args.iter().flat_map(|x| x.names()),
                x.catch_switch.names()
            ),
            Instruction::CleanupPad(x) => union!(
                x.dest.names(),
                x.args.iter().flat_map(|x| x.names()),
                x.parent_pad.names()
            ),
...
impl Names for Terminator {
    fn names(&self) -> HashSet<Name> {
        match self {
            ...
            Terminator::CondBr(x) => union!(
                x.condition.names(),
                x.false_dest.names(),
                x.true_dest.names()
            ),
            Terminator::Switch(x) => union!(
                x.operand.names(),
                x.default_dest.names(),
                x.dests
                    .iter()
                    .flat_map(|(c, n)| union!(c.names(), n.names()))
            ),
            Terminator::IndirectBr(x) => union!(
                x.operand.names(),
                x.possible_dests.iter().map(|x| x.clone())
            ),
            Terminator::Invoke(x) => x
                .function
                .as_ref()
                .right()
                .map(|x| x.names())
            ...
            Terminator::CatchSwitch(x) => union!(
                x.result.names(),
                x.parent_pad.names(),
                x.catch_handlers.clone(),
                x.default_unwind_dest
                    .as_ref()
                    .map(|x| x.names())
                    .unwrap_or_default()
            ),
...
impl Users for Name {
    fn users(&self, func: &Function, global: bool) -> Vec<LLValue> {
        let mut assigned = HashSet::<&Name>::from_iter(func.parameters.iter().map(|p| &p.name))
            .contains(self)
            || global;
        ...
        let all_instructions_terminators = func.basic_blocks.iter().flat_map(|b| {
            b.instrs
                .iter()
                .map(|x| Either::Left(x))
